...
SNAPSHOTS_PATH = ROOT / "data" / "notice-personnel-source-snapshots.json"
ASSEMBLY_PATH = ROOT / "data" / "notice-personnel-current-assembly.json"
SKELETON_PATH = ROOT / "data" / "notice-current-skeleton.json"
REPLAY_PATH = ROOT / "data" / "notice-personnel-replay-coverage.json"
...
def main() -> None:
    ...
    result = {
        "format_version": 1,
        "generator": "scripts/assemble_notice_personnel_current_candidates.py",
    ...
    print(f"Wrote {output_path.relative_to(ROOT)}: {len(output)} personnel current-text candidates")
