...
const claimRegistry = readJson(
  "docs/kaigo-ops/research/claims/claims-v0.5.json",
);
const compositionRegistry = readJson(
  "docs/kaigo-ops/research/claims/claim-compositions-v0.1.json",
);
...
const external = readJson(
  "docs/kaigo-ops/research/issues/information-search/benchmark/external-qa-query-sample-v0.2.json",
);
  ...
  JSON.stringify(
    {
      ...
      note:
        "External Q&A answer text is not used. Only the public question preview is used to stress current coverage boundaries. Claim routing metadata is authoritative where present; all current ANSWER claims have explicit routing metadata in claims-v0.5. Safe synonym expansion plus verified Claim composition resolves multi-claim cases without inventing umbrella claims. Remaining Issue fallback is reserved for claim-level coverage gaps.",
    },
    null,
    2,
  ),
