...
const claimRegistry = readJson(
  "docs/kaigo-ops/research/claims/claims-v0.24.json",
);
...
const promotion = readJson(
  "docs/kaigo-ops/research/issues/information-search/benchmark/claim-promotion-benchmark-v0.4.json",
);
...
function classify(query) {
  ...
  const preGateExplicitMatches = explicitClaimMatches(query, routed);
  const hasVerifiedAtomicAnswer = preGateExplicitMatches.some(
    (claim) =>
      claim.answerability === "ANSWER" &&
      verifiedClaimStatuses.has(claim.verification_status),
  );
  ...
  JSON.stringify(
    {
      ...
      note:
        "External Q&A answer text is not used. Only the public question preview is used to stress current coverage boundaries. Claim routing metadata is authoritative where present; all current ANSWER claims have explicit routing metadata in claims-v0.5. Verified Claim composition resolves multi-claim cases without inventing umbrella claims. Composition rules exclude queries that name a single covered subtopic, so atomic Claim routing keeps precedence in practice. The external gates use claims-v0.24 and composition-v0.5. Municipal regression uses municipal-query-sample-v0.6 (25 cases). EQ-002, EQ-003, EQ-005, and EQ-011 were promoted through source review without adding classifier special cases. Verified atomic remuneration Claims may pass the broad review gate through normal Claim routing; individual functional training, bathing-assistance, and transport-reduction subtopics are covered only by explicit atomic Claims, while unmatched remuneration mechanics remain fail-closed.",
    },
    null,
    2,
  ),
