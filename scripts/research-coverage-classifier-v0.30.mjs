...
const claimRegistry = readJson(
  "docs/kaigo-ops/research/claims/claims-v0.22.json",
);
...
const promotion = readJson(
  "docs/kaigo-ops/research/issues/information-search/benchmark/claim-promotion-benchmark-v0.2.json",
);
  ...
  JSON.stringify(
    {
      ...
      note:
        "External Q&A answer text is not used. Only the public question preview is used to stress current coverage boundaries. Claim routing metadata is authoritative where present; all current ANSWER claims have explicit routing metadata in claims-v0.5. Verified Claim composition resolves multi-claim cases without inventing umbrella claims. Composition rules exclude queries that name a single covered subtopic, so atomic Claim routing keeps precedence in practice. The external gates use claims-v0.22 and composition-v0.5. Municipal regression uses municipal-query-sample-v0.6 (25 cases). EQ-002, EQ-003, EQ-005, and EQ-011 were promoted through source review without adding classifier special cases. Verified atomic remuneration Claims may pass the broad review gate through normal Claim routing; unmatched remuneration mechanics remain fail-closed.",
    },
    null,
    2,
  ),
