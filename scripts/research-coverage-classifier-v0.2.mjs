...
const intentSignatures = {
  "manager-concurrent-role": (query) =>
    /管理者/.test(query) &&
    /(兼務|他の職務|別の仕事|別職種|別の事業所|他の事業所)/.test(query),
  ...
  "nurse-staffing": (query) =>
    /(看護職員|看護師)/.test(query) &&
    /(何人|人数|配置|常駐|事業所|訪問看護|外部|連携|駆けつけ|確保)/.test(query),

  "multiple-roles": (query) =>
    /(兼務|複数の職種|複数職種|複数の役割|別の職務|別職種)/.test(query) &&
...
function verifiedIntentMatches(query, routed) {
  return routed.filter(({ slug }) => {
    const signature = intentSignatures[slug];
    return signature ? signature(query) : false;
  });
}
...
function classify(query) {
  ...
  const intentMatches = verifiedIntentMatches(query, routed);
...
function evaluate(cases) {
  ...
  return {
    ...
    total: rows.length,
    pass_rate:
      rows.filter((row) => row.pass).length / rows.length,
...
const result = {
  ...
  design_note:
    "Similarity is used for routing only after deterministic coverage gates; an explicit verified intent signature is required before a routed Issue can become ANSWER.",
};
