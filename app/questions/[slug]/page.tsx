...
import noticeNodes from "../../../data/notice-nodes.json";
...
export default async function QuestionPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const linkedRules = isVerified && "rule_node_ids" in question ? ruleNodes.filter((node) => question.rule_node_ids?.includes(node.id)) : [];
  const linkedNotices = isVerified && "notice_node_ids" in question ? noticeNodes.filter((node) => question.notice_node_ids?.includes(node.id)) : [];
  const linkedQa = isVerified && "qa_item_ids" in question ? qaItems.filter((item) => question.qa_item_ids?.includes(item.id)) : [];

  return (
    <article className="answer-page">
      <p className="eyebrow">通所介護 / {question.category}</p>
      <h1>{question.title}</h1>
      <p className="meta">全国共通事項を対象 / 状態：<span className={isVerified ? "verified" : ""}>{isVerified ? "一次資料確認済み" : "根拠確認中"}</span>{isVerified && "last_verified" in question ? ` / 最終確認 ${question.last_verified}` : ""}</p>
      {isVerified && "verification_note" in question && question.verification_note ? <p className="scope-note">{question.verification_note}</p> : null}
      ...
      {!isVerified ? <div className="notice">この質問は現在、一次資料との対応関係を確認中です。確認が完了するまで制度上の結論は掲載しません。</div> : (
        <>
          ...
          <section className="section"><h2>実務では</h2><ol className="steps">{question.practical_steps?.map((step) => <li key={step}>{step}</li>)}</ol></section>
          <section className="section"><h2>注意</h2><ul className="source-list">{question.cautions?.map((caution) => <li key={caution}>{caution}</li>)}</ul></section>
          <section className="section">
            ...
            {"source_refs" in question ? question.source_refs?.filter((ref) => !linkedRules.some((node) => node.source_id === ref.source_id) && !linkedNotices.some((node) => node.source_ids?.includes(ref.source_id)) && !linkedQa.some((item) => item.source_id === ref.source_id)).map((ref) => {
      ...
      <section className="section"><h2>検索用の言い換え</h2><p>{question.aliases.join(" / ")}</p></section>
