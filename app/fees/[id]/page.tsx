...
export default async function FeeDetailPage({params}:{params:Promise<{id:string}>}){
  ...
  const delegatedTargets=delegatedRelations.filter(rel=>rel.from_id===node.id).map(rel=>delegated.find(item=>item.id===rel.to_id)).filter(Boolean);
      ...
      {delegatedTargets.length ? (
        <section className="section">
          ...
          <div className="relation-list">
            {delegatedTargets.map((target:any)=><div className="relation-row" key={target.id}>
              <strong>{target.source_id}</strong>
              <div><p><Link href={`/fees/criteria#${encodeURIComponent(target.id)}`}>{target.heading}</Link></p><p className="meta">取込済み・人手確認待ち</p></div>
            </div>)}
          </div>
        </section>
      ) : null}
