...
type FeeNode = {
  id:string; parent_id:string|null; number_path:string[]; title:string;
  authority_layer:string; service_scope:string; verification_status:string;
  source_id:string; source_locator:string;
  latest_amendment_evidence?:Array<{source_id:string;locator:string;evidence_type:string}>;
  note?:string;
};
type FeeText = {
  fee_id:string; official_text:string; text_sha256:string; source_id:string;
  source_url:string; source_locator:string; import_status:string;
};
    ...
    <article className="answer-page rules-page fee-detail-page">
      ...
      <div className={isReviewed ? "notice fee-reviewed-notice" : "notice"}>
        ...
        {isReviewed
          ...
          : "厚生労働省の現行HTMLから取得していますが、見出し・単位数・率・算定条件・参照先の人手照合はまだ完了していません。"}
      </div>
